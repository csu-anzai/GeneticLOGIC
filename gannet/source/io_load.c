...
if (output_file)
  ...
  while (set < conf.io_sets && error_code == SUCCESS)
    ...
    while (output_num < conf.e_outputs && error_code == SUCCESS)
      ...
      if (chr == EOF)
        {
        error_code = ERROR;
        fputs("Output file not long enough!\n",history);
        }
  ...
  fclose(output_file);
  if (error_code == SUCCESS) fputs("Output data received.\n",history); 
  } /* stop <if output_file> */
else
  {
  fputs("Output file does not exist!\n", history);
  error_code = ERROR;
  }
...
if (input_file)
  ...
  while (set < conf.io_sets && error_code == SUCCESS)
    ...
    while (input_num < conf.e_inputs && error_code == SUCCESS)
      ...
      if (chr == EOF)
        {
        error_code = ERROR;
        fputs("Input file not long enough!\n", history);
        }
  ...
  fclose(input_file);
  if (error_code == SUCCESS) fputs("Input data received.\n",history); 
  } /* stop <if input_file> */
else
  {
  fputs("Input file does not exist!\n", history);
  error_code = ERROR;
  }
...
if (combo_file != NULL)
  ...
  for(cnt = 0; cnt < COMBO4; cnt++)
    ...
    if (fscanf(combo_file,"%d %d %d %d %d %d ", &value0, &value1,
      ...
      {
      error_code = ERROR;
      fputs("File combo4.data too short!\n", history);
      break;
      }
    else
      {
  ...
  {
  error_code = ERROR;
  fputs("Unable to find combo4.data!\n", history);
  }

if (error_code == SUCCESS)
  fputs("Combination  4 information loaded.\n", history);
...
if (combo_file != NULL)
  ...
  for(cnt = 0; cnt < COMBO2; cnt++)
    ...
    if (fscanf(combo_file,"%d %d %d %d %d ",&value0, &value1,
      ...
      {
      error_code = ERROR;
      fputs("File combo2.data too short!\n", history);
      break;
      }
    else
      {
  ...
  {
  error_code = ERROR;
  fputs("Unable to find combo2.data!\n", history);
  }

if (error_code == SUCCESS)
  fputs("Combination 2 information loaded.\n",history);
