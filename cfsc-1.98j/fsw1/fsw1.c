...
/**
 LoadEnv                Load environment for BBRIGADE from specified file.
...
/*
   ...
   4, 0, 0, 0
   ; State number, followed by VisitCnt for it, followed
     ...
     FILE *FilePtr;
{
  ...
  while (ReadS (GInBuff, GIBUFFMX, FilePtr, &RetLen) != EOF)
      ...
      else
	{
	  ...
	  if (*bufptr == 'r')
	    {			/* random transition from here */
	      ++bufptr;
	      ++bufptr;		/* move past r and comma */
	  ...
	  if (tnode == NULL)
		...
		{		/* just move down list till the end is in sight */
...
#if ( CBELLMTS || LATTICEC )
  free ((char *) tnode);
#else

#if APOLLOC
  if (free ((char *) tnode) == 0)
    {				/* Apollo C free() return 0 means error (I think) */
      WriteStd ("\n**TrFree: 0 from free()\n");
      return (ERROR);
    }
#else
  ...
  if (tnode == NULL)
    {
      ...
      MadeMstk = TRUE;
      if (HiTrRsl == 0)
	{			/* Use first transitions as default */
     ...
     int BehavStp;
{
  ...
  if (BehavStp)
	...
	{			/* DftRand is on and there are > 1 setting: select effector setting at random */
...
GtSysRew ()
{
  ...
  if (CycleStp % EffectRt != 0)
    {				/* Get reward only when Generate behavior, every EffectRt steps */
      SysRew = 0.0;
      return;
    }
