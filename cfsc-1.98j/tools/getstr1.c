...
VOID Init (), GetIDPar (), AskIDs (), StoreStr (), PrntStrs (), PrntTStr (), Help ();
   ...
   /* From each file, read lines until done, filling the tables when appropriate */
	...
	while ((ret = ReadS (inbuf, sizeof (inbuf), InFptr, &len)) != EOF) {
		...
		while ((ret = ReadS (inbuf, sizeof (inbuf), InFptr, &len)) != EOF)
			...
			break;
	    } else if (strflg && strcmpn (inbuf, "Current Classifiers", 19) == 0)
		StoreStr ();
	}
...
AskIDs ()
{
    ...
    for (cfindx = 0; cfindx < NMCFSSZ; ++cfindx) {
	printf ("Cf_Id? ");
	if ((ret = ReadS (inbuf, sizeof (inbuf), stdin, &len)) == EOF || len == 0)
	    break;
	else {
	    cptr = GetInt (inbuf, &cfid, -1, " ", &err);
	    if (cfid == -1 || err)
...
StoreStr ()
{
    ...
    sscanf (inbuf, "Current Classifiers (cycle-step %d)", &step);

    strstplst[strstpndx] = step;

    ret = ReadS (inbuf, sizeof (inbuf), InFptr, &len);	/* a blank line */
    ret = ReadS (inbuf, sizeof (inbuf), InFptr, &len);	/* header line first cf */

   /* see if had header for CL,2 format */
    if (strcmpn (inbuf, "Id	S", 7) == 0) {
	while ((ret = ReadS (inbuf, sizeof (inbuf), InFptr, &len)) != EOF) {
	    strength = cfid = 0;
	    if (sscanf (inbuf, "%d %d", &cfid, &strength) != 2)
		break;
   ...
   /* see if it is format CL,22 or other 20's */
	...
	for (cp = inbuf, i = 0; *cp != '>' && i < 10 && *cp != '\0'; ++cp, ++i);
	if (*cp == '>') {
	    do {
		for (cp = inbuf, i = 0; *cp != '>' && i < 10 && *cp != '\0'; ++cp, ++i);
		if (*cp != '>')
		    break;
		GetInt (inbuf, &cfid, -1, ">", &err);
		for (; *cp != '{' && *cp != '\0'; ++cp);
		if (*cp == '\0')
		    break;
		++cp;
