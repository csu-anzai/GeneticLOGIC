...
main (argc, argv)
    ...
    char *argv[];
{
    int ret, len, err, i, filenum, idflg, id;		/* id is int  -joke */
    unsigned int sptr;
	...
	if ((Concepts[i].Concept = (char *)malloc (CONSZ)) == NULL) {
   ...
   /* From each file, read lines until done */
	...
	if (EndOnly) {			/* get only data after ENDRUN comment in log */
		...
		if (strcmpn (inbuf, "; ENDRUN", 8) == 0 ||
			strcmpn (inbuf, "C-? ; ENDRUN", 12) == 0)
		    break;
	}
	while ((ret = ReadS (inbuf, sizeof (inbuf), InFptr, &len)) != EOF) {
	   ...
	   /* in TEST region, keep CurLet up to date... */
	    ...
	    else if (TestReg && strcmpn (inbuf, "Guess", 5) == 0) {
		sscanf (inbuf, "Guess is '%c', letter is '%c'.", &CurLet, &CurLet);
	    } else if (strcmpn (inbuf, "Current Cl", 10) == 0) {
		sscanf (inbuf, "Current Classifiers (cycle-step %d) %s", &CycleStp, buff);
...
ReadCon ()
{
   ...
   /* NB--> inbuf has }rst cf in it on entry. Format is:
 ...
 180:  G:   [2] 1101###,  G: p [02] 0#0#001   //  Y:   [2] 0100010 {4198,0.81,0.0}
	...
	for (; *cp != 'm' && *cp != '1' && *cp != '#' && *cp != '\0'; ++cp);	/* get to uninterpreted classifier, if there */
