...
GenCands ()
{
    ...
    int IsMatch2, cfcnt, mcnt1, mcnt2, m1bui, m1bit, m2bui, m2bit;	/* Flags and loop counters. */
   ...
   /* get random place to start in cflist */
	...
	for (; cfcnt <= randend; ++cfcnt, ++cp) {
		...
		if (!mp1 -> IsMsg)
		    continue;		/* skip if msg removed by effector conflict resolution */

		for (i = 0; i < INTPRSTR; ++i)	/* check mp1 against condition 1 */
		    if (((mp1 -> Message[i] & cp -> Cnd1DCs[i]) ^ cp -> Cnd1Bits[i]))
			break;
		...
		if (i == INTPRSTR) {	/* Must have matched the first condition. */
			...
			for (mp2 = CurMsgs, mcnt2 = 0; mcnt2 <= LastCMsg; ++mcnt2, ++mp2) {
				...
				if (((mp2 -> Message[i] & cp -> Cnd2DCs[i]) ^ cp -> Cnd2Bits[i]))
				    break;
...
ChkEftrs ()
{
	...
	for (mcnt = 1, Mptr = NewMsgs; mcnt <= NmNMsgs; ++mcnt, ++Mptr) {
		...
		if (((Mptr -> Message[i] & ep -> ECndDCs[i]) ^ ep -> ECndBit[i]))
		    break;
