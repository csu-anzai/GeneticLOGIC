...
#include	"config.h"		/* set a switch indicating compiler/machine being used */
...
char Version[] = "1.98 (-joke)";	/* Version number	****  How to include the compilation date?? -joke says: use __DATE__ ;-) **** */
    ...
    char *CmdPars;
{
    ...
    switch (CmdCode) {
	 ...
	 break;
     case LOADC_CC:
	 LoadCfs (CmdPars, 'r');	/*  Load classifiers (replace old) */
	 break;
     case LOADM_CC:
	 LoadMsgs (CmdPars, 'r', 'c');	/* Load messages (replace old) into current list. */
	 break;
     case LOADE_CC:
	 LoadEnv (CmdPars);		/*  user supplies a file or use default */
	 break;
     case GENRC_CC:
	 GnRndCfs (CmdPars);		/*  generate random classifiers: par="how_many,meanStr,meanBR" */
	 break;
     case DISPL_CC:
	 Display (CmdPars);		/*  [parameter [,parameter]+]	 */
	 break;
     case SET_CC:
	 Set (CmdPars);			/*  parameter	value  */
	 break;
     case CLASS_CC:
	 Classify (CmdPars);		/* Run model major cycle. CmdPars indicates how may steps. */
	 break;
     case STEP_CC:
	 Classify ("1");		/* special case of RUN with  CmdPars = '1'  */
	 break;
     case CMDS_CC:
	 ExecCmds (CmdPars);		/* read and execute commands from file */
	 break;
     case ECMD_CC:
	 DoEnvCmd (CmdPars);		/* Do an environment-specific command */
	 break;
     case SAVES_CC:
	 SaveSys (CmdPars);		/* Save entire system to file */
	 break;
     case MDCF_CC:
	 Mod_Cf (CmdPars);		/* prompts for cf-id and new value */
	 break;
     case MDMSG_CC:
	 Mod_Msg (CmdPars);		/* Prompts for msg-id and new value */
	 break;
     case APCF_CC:
	 LoadCfs (CmdPars, 'a');	/* Same as Load, except append classifers to current list. */
	 break;
     case APMSG_CC:
	 LoadMsgs (CmdPars, 'a', 'c');	/* Same as Load, except append messages to current list.  */
	 break;
     case HELP_CC:
	 Help (CmdPars);
	 break;
     case NULL_CC:
	 break;				/* Don't do anything for a null line */
     default:
    ...
    char *FDName;
{
    ...
    char CmdPars[CMDPARSZ + 1], *lptr, Command[CMDNAMSZ + 1];	/* room for command name and \0 */
    FILE *fptr;
	...
	while (ReadS (GInBuff, GIBUFFSZ, fptr, &retlen) != EOF) {
	   ...
	   /* Get CmdCode, or null line command code or error command code. */
		...
		while (*lptr == ' ')
		    ++lptr;		/* skip blanks before parameter */
...
StartStp ()
{
    ...
    ActCfPNm = 0;			/* Get ready to store active classifiers in prev active list */

    for (cp = CandCfs; cp != NULL; cp = cp -> NxtCand) {
	if (cp -> NmPost > 0) {		/* Its active, so store in prev list */
	    cp -> NmPost = cp -> PstEfMsg = 0;
	    ActCfPrv[ActCfPNm].ActCf = cp;	/* store pointer to it, and */
	    ActCfPrv[ActCfPNm].ChngStr = cp -> ChngStr;	/* change in strength this step */
	    ++ActCfPNm;			/* and increment count */
	}
    ...
    char *InitPath;
{
   ...
   /*	First get some space for some global scratch variables. */
    ...
    GOutBuff = (char *)malloc (GOBUFFSZ);
    if (GOutBuff == NULL) {
	WriteStd ("\nERR (InitVT) malloc->NULL to GOutBuff!");
	fflush (stdout);
	exit (1);
    }
    GInBuff = (char *)malloc (GIBUFFSZ);
    GCfBuff = (char *)malloc (CFBUFSZ);
    GMsgBuff = (char *)malloc (MSGBUFSZ);
    if (GInBuff == NULL || GCfBuff == NULL || GMsgBuff == NULL) {
	WriteStd ("\nERR (InitVT) malloc->NULL to GInBuff &etc!");
	exit (1);
    }
    LogFName = (char *)malloc (FNAMESZ);
    GenFName = (char *)malloc (FNAMESZ);
    StdInFN = (char *)malloc (FNAMESZ);
    StdOutFN = (char *)malloc (FNAMESZ);
    DisOutFN = (char *)malloc (FNAMESZ);
    MsgInFN = (char *)malloc (FNAMESZ);
    CfInFN = (char *)malloc (FNAMESZ);
    LoadSyFN = (char *)malloc (FNAMESZ);
    SaveSyFN = (char *)malloc (FNAMESZ);
    SaveSyPa = (char *)malloc (FNAMESZ);
    EnvInFN = (char *)malloc (FNAMESZ);
   ...
   /*	ready to read lines of the form:
			...
			"L" is 'l' if variable is to be dumped/loaded on save/restart (or 0 if not);
			"S" is 's' if Special flag is to be set (or no value if not special);
			"values" is either:
	...
	if (*GInBuff == COMCHAR)
		...
		*cp = isupper (*cp) ? tolower (*cp) : *cp;
		...
		if (strlen (vflags) != 2 || (vflags[0] != 's' && vflags[0] != 'i' && vflags[0] != 'u'
		    ...
		    *cp = '\0';
		    RTVarNxt -> Name = (char *)malloc (strlen (vname) + 1);
		    ...
		    *cp = '\0';
		    RTVarNxt -> Alias = (char *)malloc (strlen (alias) + 1);
    ...
    FILE *FilePtr;
{
	...
	*lptr = '\0';
	...
	if (*lptr == '~') {
	    ...
	    if (*lptr == 'm')
		++lptr;			/* move past the m if its there */
	}
