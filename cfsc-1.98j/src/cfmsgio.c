    ...
    char How, List;
{
    ...
    if (How == 'r')
	EmptyML (List);			/* If How is 'r' (Replace), empty list. */
    ...
    int Format;
{
    ...
    if (MsgPtr -> MtchMsg1 == NULL)
	match1 = 0;			/* No match1 used by producer */
    else
	match1 = MsgPtr -> MtchMsg1 -> Msg_Id;	/* Id of match1 message */
    if (MsgPtr -> MtchMsg2 == NULL)
	match2 = 0;			/* No match2 used by producer */
    else
	match2 = MsgPtr -> MtchMsg2 -> Msg_Id;	/* Id of match2 message */
    ...
    char How;
{
    ...
    if (How == 'r')
	InitCfLs ();			/* If Replace, initialize classifier list */
...
StoreCf		Create and store new classifier in the CurCfs list.
	...
	cond1,cond2/actop/action,Strength,BidRatio,Cf_Id,ChngStr,ToTMmBid,TotMtch,
		...
		TotNmOfs,StpCrtd,StpLPrd,StpLPst,NoBBFlg,NoRplFlg,NoPrnFlg
...
StoreCf (LineBuff)
    char LineBuff[];
{
    char cond1[STRNGSZ + 1], cond2[STRNGSZ + 1], action[STRNGSZ + 1], *lbufptr, tbuff[STRNGSZ + 1], *GetFloat (), *GetUInt (),
       *GetSInt ();
    unsigned int cnd1type, cnd2type, id, actcode, totnmbid, totmtch, totprod, totpost, totemtch, toteact, totposrw, totnegrw,
        totnmofs, stpcrtd, stplprd, stplpst;
    unsigned int err;
   ...
   /* Get the second condition and (optional) condition-type prefix. */
	...
	if (*lbufptr == 'm')
	    ++lbufptr;			/* move past the m if its there */
    }
   ...
   /* Add classifier with all these values to classifier list. */
    ...
    if (AddCf (cond1, cond2, action, cnd1type, cnd2type, actcode, id, strength, bidratio,
	       ...
	       stpcrtd, stplprd, stplpst, nobbflg, norplflg, noprnflg) == ERROR) {
	return (ERROR);
    }
    return (OK);
...
int 
AddCf (Cond1, Cond2, Action, Cond1Type, Cond2Type, ActCode, Id, Str, BRatio,
       ChangeS, TNmBid, TMtch, TProd, TPost, TEMtch, TEAct, TPosRw, TNegRw, TNmOfs,
       StpC, StpLPr, StpLPs, NoBB, NoRpl, NoPrn)
    char Cond1[], Cond2[], Action[];
    unsigned int Cond1Type, Cond2Type, ActCode, Id;
    float Str, BRatio, ChangeS;
    unsigned int TNmBid, TMtch, TProd, TPost, TEMtch, TEAct, TPosRw, TNegRw, TNmOfs, StpC, StpLPr, StpLPs;
    ...
    int Format;
{
   ...
   /* zero-out all the accumulator variables. */
	...
	for (cfcnt = 1, cp = CurCfs; cfcnt <= NmCfs; ++cfcnt, ++cp) {
	    WCtotstr += cp -> Strength;
	    WCtotchng += cp -> ChngStr;
	    WCtotbr += cp -> BidRatio;
	    WCtotbid += cp -> TotNmBid;
	    WCtotmat += cp -> TotMtch;
	    WCtotprd += cp -> TotProd;
	    WCtotpst += cp -> TotPost;
	    WCtoteff += cp -> TotEAct;
	    WCtotpos += cp -> TotPosRw;
	    WCtotneg += cp -> TotNegRw;
    ...
    if (Format == DEMOFMT2) {
	if (NmCfs != 0) {
	    sprintf (GOutBuff, "\n      %6.0f %7.1f %4.2f %6u %6u %6u %6u %5u %4u %4u      %3u\n",
    ...
    int Format;
{
	...
	sprintf (OutBuf, "%-5u %6.0f %7.1f %4.2f %6u %6u %6u %6u %5u %4u %4u %4u %3u\n",
		 CP -> Cf_Id, CP -> Strength, CP -> ChngStr, CP -> BidRatio, CP -> TotNmBid, CP -> TotMtch,
		 CP -> TotProd, CP -> TotPost, CP -> TotEAct, CP -> TotPosRw, CP -> TotNegRw, CP -> StpCrtd, CP -> TotNmOfs);
    else				/* Format is SAVEFMT */
	sprintf (OutBuf, "%s%f,%f,%u,%f,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
		 ...
		 CP -> StpCrtd, CP -> StpLPrd, CP -> StpLPst,
