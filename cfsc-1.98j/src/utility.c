...
char *
GetUInt (Start, Result, Default, EndChars, Err)
    char *Start;
...
char *
GetInt (Start, Result, Default, EndChars, Err)
    char *Start;
    int *Result, Default;
    char EndChars[];
    int *Err;
...
char *
GetFloat (Start, Result, Default, EndChars, Err)
    char *Start;
...
char *
GetSInt (Start, Result, Default, EndChars, Err)
    char *Start;
...
char *
GetLInt (Start, Result, Default, EndChars, Err)
    char *Start;
...
int 
CopyChar (Destination, Source, MaxCount)
...
VOID 
SpacStrg (Destin, Source, BlckSize)
    char *Destin, *Source;
    int BlckSize;
{
    register int count;
...
char *
StpTok1 (InBuff, OutBuff, OBuffLen, ETChars)
    char InBuff[], OutBuff[];
    int OBuffLen;
    char ETChars[];
{
...
char *
StpTok2 (InBuff, OutBuff, OBuffLen, ETChars)
    char InBuff[], OutBuff[];
    int OBuffLen;
    char ETChars[];
{
...
#if MACLSC				/****** Why do I need this here when I use _MC6881_ ???? ****** */
/*
...
{
    return (_pow (x, y));
   /* _pow() does the proper error checking. */
}
...
float 
URand01 ()
{					/***** the source of all the other numbers ****/
