...
char   Procfile[40];    /* population sizes for each process    */
char   Linkfile[40];    /* file with links between processes    */
char   Regfile[40];	/* file with registered processes	*/
char   Endfile[40];	/* indicates that main has finished	*/
char   Infofile[40];    /* addr & port info of main process     */

int    My_id;              /* process id */
int    Migrecv;            /* number of migrants to receive     */
int    Migsend;		   /* number of migrants to send		*/
int    LastMigrant=0;      /* pointer to last migrant in Migrants array */
char   *Bitstring=NULL;    /* string representation of chromosome  */
double *Vector=NULL;       /* floating point representation        */
int    Genes;              /* number of interpreted genes          */
GENESTRUCT *Gene=NULL;     /* pointer to interpretation records    */
STRUCTURE  *Old=NULL;      /* pointer to population                */
STRUCTURE  *New=NULL;      /* pointer to population                */
BESTSTRUCT *Bestset=NULL;  /* set of best structures               */
LOGSTRUCT  *Log=NULL;

#if UNIX
STRUCTURE  *Migset=NULL;   /* structures eligible to migrate       */
STRUCTURE  *Migrants;      /* circular queue for migrants data     */
int        Links=0;        /* number of links to send migrants     */
int        Linkrecv=0;     /* number of links to receive migrants  */
LINKSTRUCT *Linktable=NULL;/* links to send migrants               */
LINKSTRUCT *LinkRecTable=NULL;/* links to receive migrants         */
#endif


/* The Input file specifies these parameters */
int    Processes;       /* number of participating processes    */
int    Totalexperiments;/* number of experiments                */
int    Totaltrials;     /* trials per experiment                */
int    Totalpop;        /* global population size               */
int    Popsize;         /* local population size                */
...
char   Propiniflag;	/*  proportional initialization of pop  */
char   Syncflag;	/*  synchronized migration		*/
