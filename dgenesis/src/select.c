 ...
 *		23 aug 93: incorporate migrants into population (ECP)
 */

#include "extern.h"

void Select(nNewMigrants)
int nNewMigrants;
{
	...
	if (firstflag)
	{
		sample = (int *) calloc((unsigned) Popsize + Migrecv, sizeof(int));
		if (sample == NULL)
			Error("Select: can't allocate memory for sample");
		firstflag = 0;
	}
	...
	if (Rankflag)
	{
		...
		for (i=0; i<Popsize+nNewMigrants; i++)
			Old[i].Needs_evaluation = 0;

		for (i=0; i < Popsize+nNewMigrants-1; i++)
		{
			/* find the ith best structure */
			best = -1;
			perf = 0.0;
			for (j=0; j<Popsize+nNewMigrants; j++)
			{
			...
			Old[best].Needs_evaluation = Popsize + nNewMigrants -i - 1;
		}
		/* normalizer for ranking selection probabilities */
		rank_max = 2.0 - Rank_min;
		factor = (rank_max - Rank_min) / (double) (Popsize + nNewMigrants -1);
	}
	else
	{
	...
	for (sum=i=0; i < Popsize+nNewMigrants; i++)
	{
		if (Rankflag)
		{
	...
	for (i = 0; i < Popsize + nNewMigrants; i++)
	{
		j = Randint(i, Popsize + nNewMigrants - 1);
		temp = sample[j];
		sample[j] = sample[i];
		sample[i] = temp;
	}
...
int sample[];
{
	...
	if (firstflag)
	{
		survivors = (int *) calloc((unsigned) Popsize, sizeof(int));
                if (survivors == NULL) Error("Gap: can't allocate memory for survivors");
		firstflag = 0;
	}
