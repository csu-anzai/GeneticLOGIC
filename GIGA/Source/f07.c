...
double f07(str)
/*
	Attempt number 5 to be deceptive after failure of f03 f04 f05 f06
	Ramp up to alpha/2 incorrect characters; analysis says may not be
	deceptive.
...
unsigned char str[];
{
	...
	for(i=0; i<OPTIONS.stringsize; i+=OPTIONS.alphasize) {
		...
		if (t < (OPTIONS.alphasize/2)) c += t;
		if (t >= (OPTIONS.alphasize/2)) c += (OPTIONS.alphasize - t);
	}
	return((double) c);
 }
