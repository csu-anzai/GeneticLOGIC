...
/ **************************************************************************
 ...
 This sample main() driver function implementation can be used with the
 classic binary genetic algorithm (i.e. where strings of 1's and 0's 
 are manipulated.)
 ...
 **************************************/
 init_pool (SeedPool, Pool, 0, Pool->size, bin_eval); 
 ...
 ***********************************/
 ...
 /*DEBUG*/
 print_pool (stdout, Pool, 0, Pool->size);
 pause_it();

 child = get_gene (string_length);
 ...
 for (/* CurrentGeneration already set :
	...
	/************************************************************
	 Reproduce using  'location independent crossover' operator.
	 ************************************************************/

/* HERE  NEW CALL */
     loc_recomb_cross(mom, dad, child, Pool->string_length);
