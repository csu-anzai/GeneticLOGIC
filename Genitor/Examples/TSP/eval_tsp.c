 ...
 *
 * DESCRIBE: Allocates space for 2D integer distance array
 *
 ...
 ****************************************************************************/
 ...
 get_2D_dist_array(num_points)
 {
  ...
  if (!(dists = (int **) malloc ((num_points+1)*sizeof(int *))))
	 fatal_error ("get_2D_dist_array: Malloc failure (1).\n");


  for (i=0; i<=num_points; i++)
	   if (!(dists[i] = (int *) malloc ((num_points+1)*sizeof(int))))
      ...
      for (j=1; j<i; j++)
		  {
		   ...
		   (int) ( (sqrt ((double) (DIFFSQ(xy[i][0], xy[j][0])
						         + (DIFFSQ(xy[i][1], xy[j][1])))))
                    + .5   /* add .5 to round by truncation */
                 );
