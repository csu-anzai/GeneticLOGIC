...
#include	"compiler.h"
#include	"stdio.h"
#if (LATTICEC || CBELLMTS || MPWC || SUN3 )
#include	"ctype.h"
#endif
#include "utility.h"
...
VOID  ReadEnv(), InitEnv(), ReadLetS(), SaveEnv(), MakeActS(), DsCfIE(),
	  Let2Strg(), HelpEnv(), DsplDetM(), DsCfSTag(), DsCfLet(), DsCfWnd(), DsCfAct();
double atof();
	...
	char  *FDName;
{
	...
	while ( *FDName == ' ' )  ++FDName; 			  /* strip blanks */
...
VOID  ReadEnv ( FilePtr )
	FILE   *FilePtr;
{
	int  StoreSup(), MkGuess(), EQGuess(),
		 DftGuess();						/* Make addresses resolvable */
	...
	int	Format;
{
	...
	if ( Format == SAVEFMT )
		...
		if ( DftBehFlg )  WriteS("ON.", FilePtr );	
		else			  WriteS("OFF.", FilePtr );
		WriteS( "\nDisplay NULL-Guess message is:   ", FilePtr );
		if ( NulGsMFlg )  WriteS("ON.", FilePtr );	
		else			  WriteS("OFF.", FilePtr );
	...
	float		  MIntens[];
{
	...
	for ( let=OldLet, letnum=STMemMx, nxtmsg=MStrings; letnum>=0; --letnum, let=let->NxtEnvNd, ++NumMsgs ) {
		...
		mess[STRNGSZ] = '\0';
		mess[0] = '0';	mess[1] = '0';				/* Mark as message from detector */ 	
	...
	int  Let;		
{
	...
	for ( i = 0; Punct[i] != '\0'; ++i )
		if ( Punct[i] == Let )	  break;
	if ( Punct[i] != '\0' ) 	   return( TYPEPUNC );
	else
	{	for ( i = 0; Vowels[i] != '\0'; ++i )
			if ( Let == Vowels[i] )  break;
		if ( Vowels[i] != '\0' )	 return( TYPEVOW );
		else
		{	for ( i = 0; Consonts[i] != '\0'; ++i ) 
				if ( Consonts[i] == Let )	 break;
			if ( Consonts[i] != '\0' )	 return( TYPECON );
		}
	}
	return( ERROR );
	...
	char  String[];
{
	...
	if ( type == TYPEVOW )	{
		for ( let = 0; let <= VOWMX; ++let )
		   if ( Letter == Vowels[let] )   break;
		if ( let > VOWMX )	 type = TYPENONE;
	}		

	else if ( type == TYPECON )  {
		for ( let = 0; let <= CONSMX; ++let )
		   if ( Letter == Consonts[let] )  break;
		if ( let > CONSMX )   type = TYPENONE;	
	}		

	else if ( type == TYPEPUNC )  {
		for ( let = 0; let <= PUNCMX; ++let )
		   if ( Letter == Punct[let] )	 break; 
		if ( let > PUNCMX )   type = TYPENONE;	
	}		
	...
	char  *Par;
{
	...
	if ( *cval != '\0' )  ++cval;							 /* get past the = */
	val = atof( cval ); 									 /* should be a float */
	*Par = islower( *Par ) ? toupper( *Par ) : *Par;

	if		( *Par == 'L' )	SameLRew = val;
	else if ( *Par == 'T' )	SameTRew = val;
	else if ( *Par == 'W' )	WrongRew = val;
	else if ( *Par == 'D' )	DftBehFlg = val; 
	else if ( *Par == 'N' )	NulGsMFlg = val;
	else if ( *Par == 'E' &&
		( 0 <= val && val <= 2 ) )	EfRslMch  = val;
	else {
...
VOID  DsCfSTag( CfBuff, OutBuff )   /* Display Source Tag (left 2) loci */
	char   CfBuff[], OutBuff[];
{
...
VOID  DsCfLet ( CfBuff, OutBuff )   /* Display Letter or letters */
	char CfBuff[], OutBuff[];
{
	int  i, w, let;
...
VOID  DsCfWnd ( CfBuff, OutBuff )   /* Display ``window'' counter (current 0, one-back 1, etc.) */
	char CfBuff[], OutBuff[];
{
...
VOID  DsCfAct(	CPtr, Buff )	/* Display action type */
	struct	CfNode	*CPtr;
	char			 Buff[];
{
