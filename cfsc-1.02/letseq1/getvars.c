	...
	char  *argv[];
{
	...
	short  plainflg, headflg, fullflg, cfcflg, cfwflg, cfpflg, msgcflg, msgpflg,
		   hsflg, lsflg, asflg, brflg;
...
*/
	...
	ret=ReadS( inline, sizeof(inline), InFptr, &len);   /* skip 2 lines */
	ret=ReadS( inline, sizeof(inline), InFptr, &len);

	ret=ReadS( inline, sizeof(inline), InFptr, &len);
	sscanf( inline, "Num. Candidate-classifiers: %d; %d Cfs. won (%d", 
	...
	ret=ReadS( inline, sizeof(inline), InFptr, &len);
	sscanf( inline, "Num. candidate matches: %d", &CandMsg[index] );
	ret=ReadS( inline, sizeof(inline), InFptr, &len);
	sscanf( inline, "Number of messages: %d", &PostMsg[index] );

	ret=ReadS( inline, sizeof(inline), InFptr, &len);   /* skip 1 line */
	
	ret=ReadS( inline, sizeof(inline), InFptr, &len);
	sscanf( inline, "High/Low strength Cfs: %f (%d) / %f (%d)", 
					  &f1, &i, &f2, &i );
	HiStr[index] = f1;
	LoStr[index] = f2;

	ret=ReadS( inline, sizeof(inline), InFptr, &len);
	sscanf( inline, "Total strength (ave): %f (%f)", &f1, &f2 );
	AveStr[index] = f2;

	ret=ReadS( inline, sizeof(inline), InFptr, &len);
	if ( strncmp( inline, "Average BidRatio", 16 ) == 0 ) {
			/* printf( "\nBR line '%s'", inline ); */
