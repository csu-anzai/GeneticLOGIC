...
unsigned int	CrtLen   = 100;						  /* Min number of steps at CrtFrac to meet criteria */
unsigned int	CrtStp   = -1;						   /* First step criterium met */
	...
	char  *argv[];
{
	...
	for ( filenum = 0; filenum < numfiles; ++filenum ) {
		...
		while ( (ret=ReadS( inline, sizeof(inline), InFptr, &len)) != EOF ) 
			...
			else if ( strcmpn( inline, "TotNmGsL", 8 ) == 0 ) 
				StoreGue( );
...
VOID  StoreGue ( )
{
	...
	sscanf( inline, "TotNmGsL %u, TotNmGsT %u, TotNmGsW %u (CycleStp %u) [TotNmDft %u]",
