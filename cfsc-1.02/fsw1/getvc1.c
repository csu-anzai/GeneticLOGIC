...
float  EqFrac = 0.90;   /* fraction of end of run in "Equilibrium" */
	...
	char  *argv[];
{
	...
	for ( filenum = 0; filenum < numfiles; ++filenum ) {
		...
		while ( (ret=ReadS( inline, sizeof(inline), InFptr, &len)) != EOF )
			...
			else if ( strcmpn( inline, "State", 5 ) == 0 &&
				 strcmpn( inline, "CurState  EffVal", 16 ) != 0 )
				StoreVC( );
...
VOID  StoreVC ( )
{
	...
	sscanf( inline, "State %d (r %f, tr %f) [Dft %d, Invld %d, r %d] CycleStp %ld.",
	...
	ret = ReadS( inline, sizeof(inline), InFptr, &len );  /* get Best line */
	i = 0;
	while ( strcmpn( inline, "Best:", 5 ) == 0 ) {
		cp = &inline[5];
		for ( ; i < STATESZ; ++i ) {  
...
printf("\n2. Include a 'e75' parameter to define 'equilibrium' average to be" );
printf("\n   the last 25%% of the run (default is 90, i.e., last 10%% of run." );
printf("\n3. Some limits (which could be compiled into larger values:");
