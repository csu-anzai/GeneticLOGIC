...
#define   TOTSZ		 256   /* number of test displays that can be collected */
...
short int   BooleCompress = FALSE; /* set TRUE to print most compressed form */
short int   BooleN = 0;         /* set with b parameter, as an option---see help(). 0 means don't use. */

VOID   Converge(), Getstrng(), ReadCon(), ReadConBoole(),
       PrntCon(), Help(), PrntTSta();
	...
	for ( filenum = 0; filenum < numfiles; ++filenum ) {
		...
		if ( EndRun ) {
			while ( (ret=ReadS( inline, sizeof(inline), InFptr, &len)) != EOF ) {
				if ( strcmpn( inline, "C-? ; ENDRUN", 12 ) == 0 ||
					strcmpn( inline, "; ENDRUN", 8 ) == 0 )
					break;
			}
		}
		...
		while ( (ret=ReadS( inline, sizeof(inline), InFptr, &len)) != EOF ) {
			...
			else if ( TestReg && strcmpn( inline, "State", 5 ) == 0 ) {
				sscanf( inline, "State %d", &CurState );
			}

			else if ( strcmpn( inline, "Current Cl", 10 ) == 0 ) {
				sscanf( inline, "Current Classifiers (cycle-step %f) %s", &CycleStp, buff );
...
VOID  ReadCon ( )	   
{
	...
	overflow = FALSE;
		...
		/* NB--> inline has first cf in it on entry  */
	...
	do {
		...
		if ( str == -1.0 || br == -1.0 ) {
			printf( "\ni %d, cp->%s<inline:", i, cp );
			for ( err = 0, cp = inline; *cp != '\0'; ++cp, ++err ) {
				printf( "%c", *cp );
				if ( err % 80 == 0 )
					printf( "\n" ); 
			}
...
VOID  ReadConBoole ( )
{
	...
	do {
        ...
        if ( BooleN == 0 ) {    /* just get the whole classifier */
            ...
            if ( BooleCompress ) {
                /* get rightmost BooleN bits from each condition, AND those, 
                   and only the action guess, from line like: 
                    XY  : 1#0#1#0# 00 ##0#;    G : 0##10000 #0 ##01 ==>   GD: 100#1010 01 1#0# [0]  {  419.1,0.59}
                */

                i = 0; /* start writing at beginning of con buffer */

        		for ( ; *cp != ':' && *cp != '\0'; ++cp )
	        		;   /* skip the first condition stag */

                for ( cp += 2; *cp != ' ' && *cp != '\0'; ++cp )
                    ;  /* skip to blank before start of BooleN bits of concept */

    	    	for ( ; *cp != ';' && *cp != '\0' && i < CONSZ; ++cp, ++i )  /* get whole thing to { */
	    	    	con[i] = *cp;   /* move the first condition's concept */
                con[i++] = ' ';  /* add a blank to end to go with second condition end */


        		for ( ; *cp != ':' && *cp != '\0'; ++cp )
	        		;   /* skip the second condition stag */

                for ( cp += 2; *cp != ' ' && *cp != '\0'; ++cp )
                    ;  /* skip to blank before start of BooleN bits of concept */

    	    	for ( i = 0; *cp != '=' && *cp != '\0' && i < CONSZ; ++cp, ++i ) {
                    if ( con[i] == '#' || con[i] == ' ' )   /* first is # or blank, so */
                        con[i] = *cp;   /* just overwrite---second may make is specific or leave it as is */
                    else { /* first is 1 or 0 already, so ... */
                        if (  *cp != '#' && con[i] != *cp ) /* second not a #, and not same as first... */
                            con[i] = '-';   /* mark as conflicting! */
                    }
                }

                    /* at first = of action part */

                con[i++] = '/';
                con[i++] = ' ';  /* insert 1 blank */

                for ( ; *cp != '[' && *cp != '\0'; ++cp )
                    ; /* skip to actual guess */

        		for ( ++cp; *cp != ']' && *cp != '\0' && i < CONSZ; ++cp, ++i )  /* get whole thing to { */
	        		con[i] = *cp;   /* move response */
                con[i++] = ' ';
		        con[i] = '\0';

                for ( ; *cp != '{' && *cp != '\0'; ++cp )
                    ; /* skip to strength, etc. */
            }

            else { 
                /* get rightmost BooleN bits from each condition, and only the action guess, from line like: 
                    XY  : 1#0#1#0# 00 ##0#;    G : 0##10000 #0 ##01 ==>   GD: 100#1010 01 1#0# [0]  {  419.1,0.59}
                */

        		for ( i = 0; *cp != ':' && *cp != '\0' && i < CONSZ; ++cp, ++i )
	        		con[i] = *cp;   /* move the first condition stag */
		        con[i++] = ':';
                for ( cp += 2; *cp != ' ' && *cp != '\0' && i < CONSZ; ++cp )
                    ;  /* skip to blank before start of BooleN bits of concept */
    	    	for ( ; *cp != ':' && *cp != '\0' && i < CONSZ; ++cp, ++i )  /* get whole thing to { */
	    	    	con[i] = *cp;   /* move the concept and the second cond stag */

                for ( cp += 2; *cp != ' ' && *cp != '\0' && i < CONSZ; ++cp )
                    ;  /* skip to blank before start of BooleN bits of concept */
    	    	for ( ; *cp != ':' && *cp != '\0' && i < CONSZ; ++cp, ++i )  /* get whole thing to { */
	    	    	con[i] = *cp;   /* move the concept and all to action stag */
                for ( ; *cp != '[' && *cp != '\0' && i < CONSZ; ++cp )
                    ; /* skip to actual guess */
                con[i++] = ' ';  /* insert 1 blank */
        		for ( ; *cp != '{' && *cp != '\0' && i < CONSZ; ++cp, ++i )  /* get whole thing to { */
	        		con[i] = *cp;   /* move response */
...
Converge: on entry inline has classifier line to read.
...
printf( "\nTo use with the Boole-N domains, include the parameter 'bn'," );
printf( "\nwhere the optional paramter n (which may be 6 or 11) may be" );
printf( "\nincluded to have only the rightmost n bits of each condition" );
printf( "\nextracted and used to characterize the rules, and (in both cases)" );
printf( "\nto have only the action-sides actual guess printed." );
printf( "\nInclude 'xb' to have only n bits and guess printed as concept." );
