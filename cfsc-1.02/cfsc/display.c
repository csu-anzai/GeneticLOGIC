...
VOID Display(), AuDisplay(), PrnVars(), Help();
	...
	char	*VarName, *FDName;
{
	...
	while ( *VarName == ' ' )	++VarName;	/* skip leading blanks, and cut off trailors. */
...
VOID AuDisplay ( )
{
	...
	if ( CycleStp % ADCMInt == 0 )		DsplMsgs( "", ADCMFmt, 'c' );

	if ( CycleStp % ADCfInt == 0 )		DsplCfs( "", ADCfFmt );

	if ( CycleStp % ADVarInt == 0 ) 	DsplVars( "", ADVarFmt );

	if ( CycleStp % ADEnvInt == 0 ) 	DsplEnv( "", ADEnvFmt );
	...
	unsigned int	Format;
{
	...
	extern float	SysTreas, Bid_k, BRPow, HeadTax, BidTax, PrdTaxMx, FrPayDet, CfStrDf, CfSuppDf, MsgIntDf,
					CfStrMin, CfStrMax, CfBidMin;
	...
	extern unsigned int TotCDM, TotCDMC, TotCDML, TotCEf, TotCEfW, TotCEfB, TotACPC, TotCSS, TotCSSR, TotTLB;
	...
	if ( Format == 1 || Format == 2 ) {
		...
		if ( NmCfs > 0 ) {
			...
			WriteS( "\n\nHigh/Low strength Cfs:    0.0 (0) / 0.0 (0)", FilePtr );
    		sprintf( GOutBuff,	 "\nTotal strength (ave):	%8.1f  (%.1f)", TotCfStr, AveCfStr );	
	    	WriteS( GOutBuff, FilePtr );
		...
		WriteS( GOutBuff, FilePtr );
		sprintf( GOutBuff, "\nTotCEf %6u (Wrg %d, Bd %d)   TotACPC %u  TotCSS %u TotCSSR %u  TotTLB %u",
				 TotCEf, TotCEfW, TotCEfB, TotACPC, TotCSS, TotCSSR, TotTLB );
		WriteS( GOutBuff, FilePtr );
	}
