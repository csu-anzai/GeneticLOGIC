...
The subroutines:
	...
	RCrossSt	randomly cross two strings.
	BRCrossSt	randomly cross two strings within specified loci.
...
float	URand01();
float	CalcSpec();
VOID	DscCDM(), GnrlzCf(), CDMGeo(), CrFullCf(), DCrFullCf(),
		CrCfStrg(), RCrossSt(), BRCrossSt(), Mutate(), MutatCA(), DscCEff();
...
int CndMtchS ( Msg, CndBits, CndDCs, CndType )
...
VOID GnrlzCf ( Msg, NewCf, WhichCnd )
	struct MsgNode	*Msg;
...
#define	XCND2PR	(float) 0.0			/* specify where cross will occur */
    /* If cross in action, these limit cross area to rightmost 7 bits.
       This kludge is for a demo in letseq1 to show usefulness of xover.
    */
#define BGAActCrosslb   9
#define BGAActCrosslen  7

VOID CrCfStrg ( CfPtr1, CfPtr2 )
	struct NCfNode *CfPtr1, *CfPtr2;
{
	char	 string1[STRNGSZ+1], string2[STRNGSZ+1];
	float	 randnum, CalcSpec();

	if ( (randnum = URand01()) <= XCND1PR )  {
#if CTEST
		if ( DscDemo > 0 )
		...
		BActtoA( CfPtr1->NewActB, CfPtr1->NewActD, string1 );	/* Cross the actions */
		BActtoA( CfPtr2->NewActB, CfPtr2->NewActD, string2 );
		BRCrossSt( string1, string2, BGAActCrosslb, BGAActCrosslen );/* cross the strings */
		AActtoB( string1, CfPtr1->NewActB, CfPtr1->NewActD );	/* store the crosses */
		AActtoB( string2, CfPtr2->NewActB, CfPtr2->NewActD );
...
BRCrossSt	Random Cross two Strings within specified Bounds.

	String1	|- The ascii char arrays containing the strings.
	String2	|
    LB      The left bound (counting from 0).
    Len     The length of bounded region, from 1 to STRNGSZ.

Cross will exchange from LB to LB+Len-1.

******/

VOID BRCrossSt ( String1, String2, LB, Len )
	char	String1[], String2[];
    int     LB, Len;
{
	register char	tchar;
	register int	lbd, ubd;

        /* URandN returns 0..Len-1, so get start point by... */
    lbd = LB + URandN( Len - 1 );   /* LB <= lbd <= LB + Len - 1 */
    ubd = LB + Len - 1;            /* end point is just end of region */

    if ( lbd < 0 || ubd > STRNGMX ) {
        sprintf( GOutBuff, "\n\n***BRCrossSt: cross start|end (%d,%d) out of string! No cross done.\n", lbd, ubd );
        WriteStd( GOutBuff );
        return;
    }
...
#if CTEST
	if ( DscDemo > 1 )	{
		sprintf( GOutBuff, "  Cross from %d through %d.", lbd, ubd );
		WriteStd( GOutBuff );
	}
#endif
#if GENLOG
	if ( GenFlg ) {
		sprintf( GOutBuff, "%d-%d. ", lbd, ubd );
		WriteGen( GOutBuff );
	}
#endif

	for ( ; lbd <= ubd; ++lbd ) {			/* cross them */
		tchar = String1[lbd];
		String1[lbd] = String2[lbd];
		String2[lbd] = tchar;
	}
...
struct NCfNode	*ReprodCf ( ParentCf )
