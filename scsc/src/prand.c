...
/* prand.c -- P.J. Plauger's  ANSI C random number generator routines
...
#ifndef lint
static char *rcsid = "$Id: prand.c,v 1.1 1993/10/04 12:00:09 joke Exp $";
#endif
...
{				/* compute pseudo-random value */
  ...
  return ((unsigned int) ((_Randseed >> 16) & R_MAXPRANDVAL));
}
...
randomperc ()
{
  double v = (double) prand () / ((double) R_MAXPRANDVAL + D_ONE);
  if (v == D_ZERO)
    v += R_INITIALSEED;		/* adjust */
  return ((double) v);
}
