...
;;; 15-JAN-92 gpw   Fixed problems compiling and loading into Franz Allegro CL
;;;                 version 4.1 beta.
;;;  6-SEP-92 gpw   Fixed problems compiling and loading into MCL 2.0.
...
;;; Hmmm. Since ANSI has put them back, distinguishing which post-CLtL2 variant
;;; has gotten trickier.

#+(and :excl :allegro-v4.1)
(eval-when (compile load eval)
  (multiple-value-bind (found how)
      (find-symbol "REQUIRE" :cltl1)
    (when (eq how :inherited)
      (pushnew :allegro-v4.1-final *features*))))	;I think that'l do it

#-(or (and :CMU (not :new-compiler)) :vms :mcl)
(eval-when (compile load eval)
  (unless (or (fboundp 'lisp::require) (fboundp 'user::require)
	      #+(and :excl (or :allegro-v4.0 :allegro-v4.1)
		     (not :allegro-v4.1-final)) (fboundp 'cltl1::require))
    (in-package "LISP")
...
#+(and :excl (or :allegro-v4.0 :allegro-v4.1) (not :allegro-v4.1-final) :cltl2)
(cltl1:provide 'make)
#+:allegro-v4.1-final
(provide 'make)
#-:cltl2
...
(eval-when #-:cltl2(compile load eval)
           #+:cltl2(:compile-toplevel :load-toplevel :execute)
  #+:mcl
  (ccl:provide 'make)
  #+(and :cltl2 (not (or (and :excl (or :allegro-v4.0 :allegro-v4.1)) :mcl)))
  (provide 'make)
...
(defun #-:cltl2 new-append-directories
       #+:cltl2 append-directories (absolute-dir relative-dir)
  ...
  (let* ((abs-dir (pathname absolute-dir))
    ...
    ;; :root.
    #+(or :allegro-v4.0 :allegro-v4.1)(when (eq (car abs-directory) :root) (pop abs-directory))
    #+(or :allegro-v4.0 :allegro-v4.1)(when (eq (car rel-directory) :root) (pop rel-directory))
...
(defun test-new-append-directories (&optional (test-dirs *append-dirs-tests*))
  (do* ((dir-list test-dirs (cddr dir-list))
    ...
    (format t "~&ABS: ~S ~18TREL: ~S ~41TResult: ~S"
	    abs-dir rel-dir (#-:cltl2 new-append-directories
                             #+:cltl2 append-directories abs-dir rel-dir))))
...
(defun describe-system (name &optional (stream *standard-output*) recursive)
  ...
  (unwind-protect
	...
	(multiple-value-bind (*version-dir* *version-replace*) 
	  ...
	  (let ((*load-verbose* t) ; nil
		#-:ccl-2 (*compile-file-verbose* t) ; nil
...
(unless *old-require*
  (setf *old-require* 
	(symbol-function #-(or (and :excl (or :allegro-v4.0 :allegro-v4.1)) :mcl) 'lisp:require
			 #+(and :excl (or :allegro-v4.0 :allegro-v4.1)
				(not :allegro-v4.1-final)) 'cltl1:require
			 #+:allegro-v4.1-final 'require
			 #+:mcl 'ccl:require))
  ...
  (unless *dont-redefine-require*
    (let (#+:CCL (ccl:*warn-if-redefine-kernel* nil))
      ...
      (setf (symbol-function 
	     #-(or (and :excl (or :allegro-v4.0 :allegro-v4.1)) :mcl) 'lisp:require
	     #+(and :excl :allegro-v4.0) 'cltl1:require
	     #+:mcl 'ccl:require)
	    (symbol-function 'new-require))
      #+(and :excl :allegro-v4.1) 
      (handler-bind ((excl:package-locked-error
		      #'(lambda (ignore)
			  (declare (ignore ignore))
			  (invoke-restart (first (compute-restarts))))))
		    (setf (symbol-function #-:allegro-v4.1-final 'cltl1:require
					   #+:allegro-v4.1-final 'require)
			  (symbol-function 'new-require))))))
