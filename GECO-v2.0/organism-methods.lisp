  ...
  (and (eq (class-of org1) (class-of org2))
       (or (and (null genes-1) (null genes-2))
           (do* ((g1-ptr genes-1 (rest g1-ptr))
                 ...
                 same) ; won't be tested first time
                ((or (null g1-ptr) (not same))
                 same)
             (setq same (eidetic (first g1-ptr) (first g2-ptr)))))))
...
(defmethod PICK-RANDOM-CHROMOSOME-INDEX ((self organism))
  (do* ((sizes (mapcar #'size (genotype self)))
        ...
        (sum 0))
       ((or (null s-list)
            (>= (geco-random-integer n-genes) sum))
        c#)
...
(defmethod 2X-CROSS-ORGANISMS ((parent1 organism) (parent2 organism)
                               ...
                               (parent1-chromosome (nth chromosome-index
                                                        (genotype parent1)))
                               (locus-index1 (pick-locus-index
                                              parent1-chromosome))
                               (locus-index2 (pick-locus-index
                                              parent1-chromosome)))
...
(defmethod PMX-CROSS-ORGANISMS ((parent1 organism) (parent2 organism)
                                 ...
                                 (parent1-chromosome (nth chromosome-index
                                                          (genotype parent1)))
                                 (locus-index1 (pick-locus-index parent1-chromosome))
                                 (locus-index2 (pick-locus-index parent1-chromosome)))
