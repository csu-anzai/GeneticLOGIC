...
// Modified from GNU class library by Anthony Tang Yiu Cheung (CUHK)
...
protected:
 ...
 public:
                    BitStrBit(BitString& v, int p) :src(v), pos(p) {}
...
protected:
  ...
  BitSubString       _substr(int first, int l) const;
...
// BitSubString extraction
  ...
  BitSubString       at(int pos, int len) const;
...
// added by Anthony Tang (CUHK) for TOLKIEN

    //
    // gray code conversion
    //
    BitString&  toGray();       // convert binary to gray
    BitString&  deGray();       // convert gray to binary

    //
    // 	BitString to numeric conversion
    //
    float 	    bin2float(int, int) const;
    unsigned long   bin2long(int, int) const;
    unsigned long   asLong() const;

    // crossover operators
    void oddPtCrossover(const BitString&, const BitString&, int);
    void evenPtCrossover(const BitString&, const BitString&, int);
    void uniformCrossover(const BitString&, const BitString&, float);

    //
    unsigned        bitsum() const;
};
...
inline 	void BitString::resize(const int nLength)
{
	rep = BStr_resize(rep, nLength);
}
...
union	BStrValue
//
//	union used for fast conversion from BitString to long
//
{
	unsigned long  lVal;
	unsigned short uVal[2];
};

inline	unsigned long BitStrRepasLong(BitStrRep *rep)
//
//	convert BitStrRep to an unsigned long
//
{
	BStrValue	bval;
