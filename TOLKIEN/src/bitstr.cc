...
void  bcopy(const void* s, void* t, int l);
...
inline void  bcopy(const void* s, void* t, int l) { memcpy(t,s,l); }
...
#define MAXBitStrRep_SIZE  ((unsigned int) (1 << (SHORTBITS - 1)) - 1)
...
#define MAXBIT  ((unsigned short) (1 << (BITSTRBITS - 1)))
...
BitSubString BitString::at(int first, int len) const
{
...
const char* BitStringtoa(const BitString& x, char f, char t)
{
  ...
  char* fmtbase = (char *) malloc(wrksiz);
  char* fmt = fmtbase;
  unsigned int  xl = x.rep->len;
...
BitSubString BitString::_substr(int first, int l) const
{
  if (first < 0 || l <= 0 || (unsigned)(first + l) > rep->len)
	return BitSubString(_nil_BitString, 0, 0) ;
  else
        return BitSubString(* (BitString *) this, first, l);
}

BitString&  BitString::toGray() 	// convert binary to gray
{
	BitString       grayBits = *this;
	register int i;
	register int last;

	for (i = length() - 1, last = 0; i >= 0; i--) {
	     grayBits.assign(i, test(i) != last);
	     last = test(i);
	}
...
BitString&  BitString::deGray()        // convert gray to binary
{
	BitString       binaryBits = *this;
	register int i;
	register int last;

	for (i = length() - 1, last = 0; i >= 0; i--) {
		if (test(i))
		    !last ? binaryBits.set(i) : binaryBits.clear(i);
		else
		    last ? binaryBits.set(i) : binaryBits.clear(i);
		last = binaryBits.test(i);
	}
...
float BitString::bin2float(int nStartPos, int nLength) const
{
    if (nLength <= 0)
	return 0;

    register unsigned long wPower2 = 1;
    register int nI, nJ, nL, nP;
    register float flCoeff, flAccum, flPower;
    BitString bstr;

    flCoeff = pow(2.0, (double) LONGBITS);

    if (nStartPos < 0)
	nI = 0;
    else
	nI = (nStartPos > length() - 1) ? 0 : nStartPos;

    if (nLength < 0)
	nJ = 1;
    else
	nJ = (nLength > (nJ = length() - nI)) ? nJ : nLength;
    ...
    int nWords = ceil(((double) nJ) / LONGBITS);

    unsigned long *pWords = new unsigned long [nWords];

    for (nP=nWords-1;nP>=0; nP--)
	 // clear all bits
	 pWords[nP] = 0L;
    ...
    for (nI=0, flPower = 1, flAccum=0; nI < nWords; flPower *= flCoeff, nI++)
	flAccum += (flPower * pWords[nI]);
...
unsigned long   BitString::bin2long(int nStartPos, int nLength) const
{
    if (nLength <= 0)
	return 0;

    BitString      bstr = at(nStartPos, nLength);

    if (bstr.length() < LONGBITS)
	return bstr.asLong();
    else
	return LONG_MAX; 	// overflow
}

unsigned long   BitString::asLong() const
{
	check_last(rep);
	return BitStrRepasLong(rep);
}

void    BitString::evenPtCrossover(const BitString& mother,
                                   const BitString& father, int nXpts)
//
//      no checking on the length of the strings is performed
//      because this is assumed to be the task of the class Crossover
//
//      it is assumed that nXpts is an even number
//
//      so if length of father = length of mother = 1
//      a runtime error will occur
//
{
        BitString *pShorter, *pLonger;
        int i, j, k, len;
	BOOL   flag;

	if (mother.length() >= father.length()) {
	    pLonger = (BitString*) &mother;
	    pShorter = (BitString*) &father;
	}
	else {
	    pLonger =(BitString*) &father;
	    pShorter = (BitString*) &mother;
	}
...
void    BitString::oddPtCrossover(const BitString& mother,
				  const BitString& father, int nXpt)
//
//      no checking on the length of the strings is performed
//      because this is assumed to be the task of the class Crossover
//
...
//
{
        BitString *pCurMate, *pNextMate, *pTemp;
	int cur, i, j, k, select, remaining;
        BOOL flag;

        if (mother.length() >= father.length()) {
            pNextMate = (BitString*) &mother;
            pCurMate = (BitString*) &father;
        }
        else {
            pNextMate = (BitString*) &father;
            pCurMate = (BitString*) &mother;
        }
...
void    BitString::uniformCrossover(const BitString& mother,
				    const BitString& father,
				    float flXRate)
{
	    resize(mother.length());
	    for (register int nI = 0 ; nI < mother.length(); nI++)
		    if (tRand.flip(flXRate))
			assign(nI, mother.test(nI));
		    else
			assign(nI, father.test(nI));
}

unsigned  BitString::bitsum() const
{
  check_last(rep);
  int xwds = BitStr_len(rep->len);
  int xlast = BitStr_pos(rep->len);
